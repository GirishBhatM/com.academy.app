...
<g:form class="form-horizontal">
	<div class="form-group">
		...
		<div class="col-xs-10">
			<input type="number" name="sh" min="1" max="12" required
				value="${slot.startTime.split(":")[0] }"> <input
				type="number" name="sm" min="0" max="59" required
				value="${slot.startTime.split(":")[1] }">
			<g:select name="st" from="${TimeType.values() }"
				value="${slot.sType.toString() }" />
		</div>
	</div>
	<div class="form-group">
		<label class="control-label col-xs-2">End Time:</label>
		<div class="col-xs-10">
			<input type="number" name="eh" min="1" max="12" required
				value="${slot.endTime.split(":")[0] }"> <input type="number"
				name="em" min="0" max="59" required required
				value="${slot.endTime.split(":")[1] }">
			<g:select name="et" from="${TimeType.values() }"
				value="${slot.eType.toString() }" />
		</div>
	</div>
	<div class="form-group">
		...
		<div class="col-xs-10">
			<g:each in="${Day.values()}" var="day">
				...
				<g:checkBox name="days" value="${day}"
					checked="${slot.days.contains(day) }" />
			</g:each>
		</div>
	</div>
	<div class="form-group">
		<div class="col-xs-offset-2 col-xs-10">
			<g:submitToRemote value="Update" controller="slot" action="update"
